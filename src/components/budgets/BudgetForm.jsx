  ...
  return (
    <form onSubmit={handleSubmit}>
      <div style={{ display: "grid", gap: "1rem" }}>
        <div>
          {label("Category")}
          <select className="input-base" value={form.category} onChange={(e) => setField("category", e.target.value)} disabled={!!initialData} style={{ cursor: initialData ? "not-allowed" : "pointer" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.75rem" }}>
          <div>
            ...
            <input className="input-base" type="number" min="1" step="0.01" value={form.monthly_limit} onChange={(e) => setField("monthly_limit", e.target.value)} placeholder="0.00" />
          </div>
          <div>
            {label("Alert Threshold (%)")}
            <input className="input-base" type="number" min="1" max="99" value={form.alert_threshold} onChange={(e) => setField("alert_threshold", parseInt(e.target.value))} placeholder="80" />
          </div>
        </div>
